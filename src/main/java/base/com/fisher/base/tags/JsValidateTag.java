...
public class JsValidateTag extends AbstractFormTag {
	...
	public String getModelAttribute() throws JspException {
		String resolvedModelAttribute = (String) evaluate("modelAttribute",
				this.modelAttribute);
	...
	protected int writeTagContent(TagWriter tagWriter) throws JspException {
		...
		if (getRequestContext().getModel() != null)
			...
			model = this.pageContext.getRequest().getAttribute(
					getModelAttribute());
		if (model != null) {
			...
			if (fieldValidateMap.size() > 0) {
				...
				int i = 0;
				Iterator<Entry<String, List<String[]>>> iterator = fieldValidateMap
						.entrySet().iterator();
				while (iterator.hasNext()) {
					...
					for (String[] array : entry.getValue()) {
						rulesBuilder.append(array[0]).append(":")
								.append(array[1]);
						messagesBuilder.append(array[0]).append(":\"")
				...
				// 在失去焦点时验证
				tagWriter
						.appendValue("onfocusout:function(element){$(element).valid();},");
